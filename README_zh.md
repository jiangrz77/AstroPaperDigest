...
> **注意：** 请勿在 `~/Downloads/` 下运行——macOS 会阻止下载的文件。请先将项目移动到固定位置（如 `~/Projects/` 或 `~/Applications/`）。

仓库不包含 `.app` 文件，需先构建：

```bash
./build_app.sh   # 创建 .venv、安装依赖、构建 .app
```
...
> **macOS 安全提示？** 首次启动时 macOS 可能阻止应用。前往 **系统设置 → 隐私与安全性**，点击 **“仍要打开”**。仅需操作一次。
